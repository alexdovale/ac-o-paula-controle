// js/ui.js - CORE VISUAL E MOTOR DE RENDERIZAÇÃO (PADRÃO SaaS CORPORATIVO)
...
export const UIService = {
    ...
    preencherListaColaboradoresModal(app) {
        ...
        btnNaoAtribuir.className = "collaborator-item w-full text-left p-4 mb-2 bg-white border-2 border-slate-200 rounded-xl hover:bg-slate-50 transition-all shadow-sm flex items-center gap-3";
        ...
        if (colabs.length === 0) {
            ...
            msg.className = 'text-center text-slate-400 py-4 text-sm';
            msg.innerHTML = `
            ...
            colabs.forEach(c => {
                try {
                    ...
                    btn.className = "collaborator-item w-full text-left p-4 mb-2 bg-white border border-slate-200 rounded-xl hover:border-blue-500 transition-all shadow-sm flex items-center gap-3";
    ...
    renderPautaFilters(containerId, activeFilter, onFilterChange, app) {
        ...
        const dateFiltersHTML = `
            <div id="periodo-filters-container" class="flex flex-wrap gap-4 mt-4 p-4 bg-slate-50 rounded-xl border border-slate-200 ${isPeriodo ? '' : 'hidden'} animate-fade-in">
                ...
                <div class="flex items-end">
                    <button id="aplicar-filtro-periodo" class="bg-blue-600 text-white px-6 py-2 rounded-lg font-medium hover:bg-blue-700 transition shadow-sm w-full sm:w-auto">
                        Aplicar Filtro
                    </button>
                </div>
            </div>
        ...
        const unidadesFiltersHTML = !isEventoMode ? `
            <div id="unidades-filters-container" class="flex flex-wrap gap-4 mt-4 p-4 bg-slate-50 rounded-xl border border-slate-200 ${isUnidades ? '' : 'hidden'} animate-fade-in">
                ...
                <div class="flex items-end">
                    <button id="aplicar-filtro-unidades" class="bg-blue-600 text-white px-6 py-2 rounded-lg font-medium hover:bg-blue-700 transition shadow-sm w-full sm:w-auto">
                        Buscar Pautas
                    </button>
                </div>
            </div>
    ...
    renderAssistedLists(app) {
        ...
        if (allAssisted.length === 0) {
            ...
            if (pautaList) pautaList.innerHTML = '<p class="text-slate-400 text-center p-4 text-xs font-medium">Nenhum agendamento</p>';
            if (aguardandoList) aguardandoList.innerHTML = '<p class="text-slate-400 text-center p-4 text-xs font-medium">Ninguém aguardando</p>';
            if (emAtendimentoList) emAtendimentoList.innerHTML = '<p class="text-slate-400 text-center p-4 text-xs font-medium">Ninguém em atendimento</p>';
            if (atendidosList) atendidosList.innerHTML = '<p class="text-slate-400 text-center p-4 text-xs font-medium">Nenhum atendido</p>';
            if (faltososList) faltososList.innerHTML = '<p class="text-slate-400 text-center p-4 text-xs font-medium">Nenhum faltoso</p>';
            if (distribuicaoList) distribuicaoList.innerHTML = '<p class="text-slate-400 text-center p-4 text-xs font-medium">Nenhum aguardando distribuição</p>';
    ...
    renderPautaColumn(items) {
        ...
        if (items.length === 0) {
            container.innerHTML = '<p class="text-slate-400 text-center p-4 text-xs font-medium">Nenhum agendamento</p>';
            return;
        }
        ...
        card.innerHTML = `
            ...
            <div class="mt-4 space-y-2">
                <div class="grid grid-cols-2 gap-2">
                    <button data-id="${item.id}" class="check-in-btn bg-blue-600 hover:bg-blue-700 text-white font-medium py-2 rounded-lg text-xs transition-colors shadow-sm w-full">
                        Marcar Chegada
                    </button>
                    <button data-id="${item.id}" class="faltou-btn bg-white border border-slate-300 hover:bg-slate-50 text-slate-700 font-medium py-2 rounded-lg text-xs transition-colors shadow-sm w-full" ${canEdit ? '' : 'disabled'}>
                        Faltou
                    </button>
                </div>
                <button data-id="${item.id}" class="edit-assisted-btn w-full text-blue-600 hover:text-blue-700 hover:bg-blue-50 font-medium py-1.5 rounded-lg text-xs transition-colors" ${canEdit ? '' : 'disabled'}>
                    Editar Dados
                </button>
            </div>
    ...
    renderAguardandoColumn(items, currentPautaData, colaboradores) {
        ...
        if (items.length === 0) {
            container.innerHTML = '<p class="text-slate-400 text-center p-4 text-xs font-medium">Nenhum agendamento aguardando</p>';
            return;
        }
    ...
    createAguardandoCard(item, currentPautaData, colaboradores, index) {
        try {
            ...
            const atenderButton = canAttend
                ? `<button data-id="${item.id}" data-name="${escapeHTML(nomeSeguro)}" class="${currentPautaData?.useDelegationFlow ? 'select-collaborator-btn' : 'attend-directly-from-aguardando-btn'} bg-blue-600 text-white font-medium py-2 px-3 rounded-lg hover:bg-blue-700 text-xs shadow-sm transition-colors w-full">Atender</button>`
                : '';
    ...
    renderEmAtendimentoColumn(items, currentPautaData, pautaId, userName) {
        ...
        if (items.length === 0) {
            container.innerHTML = '<p class="text-slate-400 text-center p-4 text-xs font-medium">Ninguém em atendimento</p>';
            return;
        }
    ...
    createEmAtendimentoCard(item, currentPautaData, pautaId, userName, index) {
        try {
            ...
            const delegateBtnClass = isDelegated ? 'bg-indigo-50 border-indigo-200 text-indigo-400 cursor-not-allowed' : 'bg-white border-slate-300 text-slate-700 hover:bg-slate-50';
            ...
            const buttonsContainerHtml = canDelegateOrFinalize
                ? `<div class="mt-4 flex flex-col gap-2">
                        <div class="grid grid-cols-2 gap-2">
                            <button id="btn-delegar-card" data-id="${item.id}" data-name="${escapeHTML(item.name || '')}" data-collaborator-name="${escapeHTML(atendenteNome)}" class="select-collaborator-btn ${delegateBtnClass} border font-medium py-2 rounded-lg text-xs shadow-sm transition-colors" ${canDelegate ? '' : 'disabled'}>
                                Delegar
                            </button>
                            <button data-id="${item.id}" class="attend-directly-from-aguardando-btn bg-blue-600 text-white font-medium py-2 rounded-lg text-xs shadow-sm transition-colors hover:bg-blue-700">
                                Finalizar
                            </button>
                        </div>
                        <button data-id="${item.id}" class="return-to-aguardando-from-emAtendimento-btn bg-white border border-slate-200 text-slate-600 hover:bg-slate-50 font-medium py-2 rounded-lg text-xs shadow-sm transition-colors">
                            Voltar para Fila
                        </button>
    ...
    renderAtendidosColumn(items) {
        ...
        if (items.length === 0) {
            container.innerHTML = '<p class="text-slate-400 text-center p-4 text-xs font-medium">Nenhum atendido</p>';
            return;
        }
        ...
        try {
            ...
            card.innerHTML = `
                <div class="flex justify-between items-start mb-2">
                    ...
                    <button data-id="${item.id}" class="toggle-confirmed-atendido w-7 h-7 rounded-full border flex items-center justify-center ${confirmButton} transition-colors flex-shrink-0" ${canToggleConfirmed ? '' : 'disabled'} title="Status">
                ...
                <div class="pt-3 border-t border-slate-100">
                    <button data-id="${item.id}" class="return-from-atendido-btn w-full bg-white border border-slate-200 text-slate-600 font-medium py-2 rounded-lg text-xs shadow-sm hover:bg-slate-50 transition-colors" ${canRevert ? '' : 'disabled'}>
                        Mover de Volta
                    </button>
                </div>
                ${this._getStandardizedFooterHtml(item)}
    ...
    renderFaltososColumn(items) {
        ...
        if (items.length === 0) {
            container.innerHTML = '<p class="text-slate-400 text-center p-4 text-xs font-medium">Nenhum faltoso</p>';
            return;
        }
        ...
        items.forEach(item => {
            ...
            card.innerHTML = `
                <div class="flex justify-between items-start mb-2">
                    ...
                    <button data-id="${item.id}" class="toggle-confirmed-faltoso w-7 h-7 rounded-full border flex items-center justify-center ${confirmButtonClass} shadow-sm transition-colors flex-shrink-0" ${canToggleConfirmed ? '' : 'disabled'} title="Lançar falta no Verde">
                        <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 16 16" fill="currentColor">
                            <path d="M12.736 3.97a.733.733 0 0 1 1.047 0c.286.289.29.756.01.105L7.882 12.5a.733.733 0 0 1-1.065.04L3.257 8.375a.733.733 0 0 1 1.064-.04l2.254 2.255Z"/>
                        </svg>
                    </button>
                ...
                <div class="pt-3 border-t border-slate-100">
                    <button data-id="${item.id}" class="return-to-pauta-from-faltoso-btn w-full bg-white border border-slate-200 text-slate-600 font-medium py-2 px-4 rounded-lg text-xs shadow-sm hover:bg-slate-50 transition-colors" ${canRevert ? '' : 'disabled'}>
                        Reativar Assistido
                    </button>
                </div>
    ...
    renderDistribuicaoColumn(items, pautaId, userName) {
        ...
        if (items.length === 0) {
            container.innerHTML = '<p class="text-slate-400 text-center p-4 text-xs font-medium">Nenhum aguardando distribuição</p>';
            return;
        }
        ...
        Object.keys(groups).forEach(defensor => {
            ...
            const headerHtml = `
                <div class="bg-white p-3 border-b border-cyan-200 flex flex-col gap-2">
                    ...
                    <button onclick="navigator.clipboard.writeText('${linkPainel}'); window.showNotification('Link do painel copiado!', 'success');" class="w-full bg-cyan-50 border border-cyan-200 text-cyan-700 hover:bg-cyan-100 text-xs font-medium py-1.5 rounded-lg shadow-sm flex items-center justify-center gap-1.5 transition-colors">
            ...
            groups[defensor].forEach((item, index) => {
                ...
                const actionControlsHtml = canManageDistribution
                    ? `<div class="mt-4 flex flex-col gap-2">
                            <div class="grid grid-cols-2 gap-2">
                                <button onclick="window.open('${linkExterno}', '_blank')" class="w-full bg-cyan-600 hover:bg-cyan-700 text-white font-medium py-2 rounded-lg text-xs shadow-sm transition-colors">
                                    Abrir Link
                                </button>
                                <button data-id="${item.id}" class="delegate-finalization-btn bg-green-600 hover:bg-green-700 text-white font-medium py-2 rounded-lg text-xs shadow-sm transition-colors">
                                    Concluir
                                </button>
                            </div>
                            <button data-id="${item.id}" class="return-to-aguardando-from-dist-btn w-full bg-white border border-slate-200 text-slate-600 hover:bg-slate-50 font-medium py-2 rounded-lg text-xs shadow-sm transition-colors">
                                Reverter para Fila
                            </button>
                       </div>`
                    : `<div class="mt-4">
            ...
            <div class="bg-white p-5 sm:p-8 rounded-2xl shadow-xl w-full max-w-2xl relative flex flex-col border border-slate-100" style="max-height: 95vh;" onclick="event.stopPropagation()">
                ...
                <div class="flex-grow overflow-y-auto scrollable-content pr-2 sm:pr-4 text-slate-600">
                    <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-3 gap-2">
                         ...
                         <button id="copy-format-btn" class="bg-white border border-slate-200 text-slate-600 text-xs font-medium py-1.5 px-3 rounded-lg hover:bg-slate-50 w-full sm:w-auto transition-colors shadow-sm">Copiar Formato</button>
                    </div>
                    ...
                    <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-3 gap-2">
                        ...
                        <button id="copy-example-btn" class="bg-white border border-slate-200 text-slate-600 text-xs font-medium py-1.5 px-3 rounded-lg hover:bg-slate-50 w-full sm:w-auto transition-colors shadow-sm">Copiar Exemplo</button>
                    </div>
...
67890;Fulano de Tal;10:00;Curatela;444.555.666-77</code></pre>
                    ...
                    <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-3 gap-2 pt-5 border-t border-slate-100">
                        ...
                        <button id="copy-prompt-btn" class="bg-white border border-slate-200 text-slate-600 text-xs font-medium py-1.5 px-3 rounded-lg hover:bg-slate-50 w-full sm:w-auto transition-colors shadow-sm">Copiar Prompt</button>
                    </div>
...
Por favor, me entregue o texto pronto para que eu possa salvar em um arquivo .csv.</code></pre>
            ...
            <div class="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden transform scale-100 transition-transform border border-slate-100">
                <div class="p-6 text-center">
                    ...
                    <div class="flex flex-col gap-2">
                        <button id="expired-stats-btn" class="w-full bg-blue-600 hover:bg-blue-700 text-white font-medium py-2.5 px-4 rounded-lg transition-colors shadow-sm flex items-center justify-center gap-2">
                            Abrir Estatísticas / PDFs
                        </button>
                        <button id="expired-cancel-btn" class="w-full bg-white border border-slate-200 hover:bg-slate-50 text-slate-600 font-medium py-2.5 px-4 rounded-lg transition-colors shadow-sm">
                            Voltar
                        </button>
                    </div>
                </div>
    ...
    renderPautaCards(pautas, userId, userEmail, app) {
        ...
        if (!pautas || pautas.length === 0) {
            container.innerHTML = '<p class="col-span-full text-center py-8 text-slate-500 font-medium">Nenhuma pauta encontrada.</p>';
            return;
        }
        ...
        pautas.forEach(pauta => {
            ...
            const card = document.createElement('div');
            card.className = `relative bg-white rounded-xl shadow-sm border border-slate-200 transition-all group ${isExpired ? 'opacity-60 grayscale-[0.5] cursor-not-allowed' : 'cursor-pointer hover:shadow-md hover:border-slate-300'} ${isClosed ? 'opacity-70' : ''}`;
            ...
            if (isEvento) {
                ...
                originHtml = `<h2 class="text-xs font-bold text-amber-700 bg-amber-50 border border-amber-200 px-2 py-0.5 rounded flex items-center gap-1 w-max"><span></span> ${tipoLabel}</h2>`;
            } else {
                const nomeUnidade = pauta.unidadeNome || pauta.origin || pauta.orgao;
                originHtml = nomeUnidade
                    ? `<h2 class="text-xs font-bold text-indigo-700 bg-indigo-50 border border-indigo-200 px-2 py-0.5 rounded flex items-center gap-1 w-max"><span></span> ${escapeHTML(nomeUnidade)}</h2>`
                    : `<h2 class="text-xs font-bold text-slate-500 bg-slate-50 border border-slate-200 px-2 py-0.5 rounded flex items-center gap-1 w-max"><span></span> Pauta Normal</h2>`;
            }
            ...
            card.innerHTML = `
                <div class="p-5">
                    <div class="flex justify-between items-start mb-3">
                        ${originHtml}
                        ${isOwner ? `
                        <button class="delete-pauta-btn text-slate-300 hover:text-red-500 hover:bg-red-50 p-1 rounded-md transition-colors z-20 opacity-0 group-hover:opacity-100">
                    ...
                    <p class="text-slate-800 font-semibold text-base mb-1 truncate" title="${escapeHTML(pauta.name)}">${escapeHTML(pauta.name)}</p>
                    <p class="text-xs text-slate-500 mb-4">👥 Membros associados: <span class="font-medium">${pauta.members ? pauta.members.length : 1}</span></p>

                    <div>
                        ${isOwner ?
                            `<span class="inline-flex items-center px-2 py-0.5 rounded text-[10px] font-medium bg-emerald-50 text-emerald-700 border border-emerald-200">Criador</span>` :
                            `<span class="inline-flex items-center px-2 py-0.5 rounded text-[10px] font-medium bg-blue-50 text-blue-700 border border-blue-200">Compartilhada</span>`
                        }
                    </div>
                </div>

                <div class="bg-slate-50 px-5 py-3 border-t border-slate-100 grid grid-cols-2 gap-4">
                    <div>
                        <p class="text-[9px] text-slate-400 font-semibold uppercase tracking-wider mb-0.5">Criada em</p>
                        <p class="text-[11px] font-medium text-slate-700">${dataCriacaoStr}</p>
                    </div>
                    <div>
                        <p class="text-[9px] ${isExpired ? 'text-red-500' : 'text-amber-500'} font-semibold uppercase tracking-wider mb-0.5">Eliminação</p>
                        <p class="text-[11px] font-medium text-slate-700">${dataExpiracaoStr}</p>
                    </div>
                </div>
            `;
    ...
    setupColumnControls(app) {
        ...
        columns.forEach(col => {
            ...
            if (container && countBadge && !container.hasAttribute('data-controls-added')) {
                ...
                btnGroup.innerHTML = `
                    <button class="btn-maximize bg-white hover:bg-slate-100 border border-slate-200 text-slate-500 hover:text-slate-700 p-1.5 rounded-lg transition-colors shadow-sm" title="Expandir">
                    ...
                    <button class="btn-popout bg-white hover:bg-slate-100 border border-slate-200 text-blue-600 hover:text-blue-700 p-1.5 rounded-lg transition-colors shadow-sm" title="Monitor Secundário">
