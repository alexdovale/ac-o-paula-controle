...
export const UIService = {
    ...
    preencherListaColaboradoresModal(app) {
        ...
        // BOTÃO NÃO ATRIBUIR (Design Executivo)
    ...
    // MENU DE AÇÕES PADRONIZADO (Executivo)
    ...
    createPautaCard(item) {
        ...
        card.innerHTML = `
            ${canDelete ? `
            <button data-id="${item.id}" class="delete-btn absolute top-2 left-2 text-slate-300 hover:text-red-500 transition-colors bg-white rounded-full" ${isOwner ? '' : 'disabled'} title="Excluir (apenas criador)">
            ...
            <div class="px-2 pt-2">
                <p class="font-bold text-lg text-slate-800 leading-tight uppercase mb-1.5">${escapeHTML(nomeSeguro).toUpperCase()}</p>
        ...
        try {
            ...
            if (currentPautaData?.type === 'multisala') {
                ...
                if (availableRooms.length > 0 && canEditPriority) {
                    ...
                    roomDropdownHtml = `
                        <div class="flex items-center justify-center gap-1.5 mt-2 bg-purple-50 p-1.5 rounded-lg border border-purple-100">
                    ...
                    roomDropdownHtml = `
                        <div class="flex items-center justify-center gap-1.5 mt-2 bg-purple-50 p-1.5 rounded-lg border border-purple-100 w-max mx-auto">
            ...
            const numeroBadge = `
                <div class="absolute -left-2 -top-2 w-8 h-8 bg-emerald-600 text-white rounded-full flex items-center justify-center font-bold text-sm shadow-lg border-2 border-white z-20">
                    ${numeroOrdem}
                </div>
            `;
            ...
            card.innerHTML = `
                ...
                ${canDelete ? `
                <button data-id="${item.id}" class="delete-btn absolute top-2 left-8 text-slate-300 hover:text-red-500 p-1 rounded-full transition-colors">
                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" viewBox="0 0 16 16">
                        <path d="M11 1.5v1h3.5a.5.5 0 0 1 0 1h-.538l-.853 10.66A2 2 0 0 1 11.115 16h-6.23a2 2 0 0 1-1.994-1.84L2.038 3.5H1.5a.5.5 0 0 1 0-1H5v-1A1.5 1.5 0 0 1 6.5 0h3A1.5 1.5 0 0 1 11 1.5Zm-5 0v1h4v-1a.5.5 0 0 0-.5-.5h-3a.5.5 0 0 0-.5.5ZM4.5 5.029l.5 8.5a.5.5 0 1 0 .998-.06l-.5-8.5a.5.5 0 1 0-.998.06Zm3 0l.5 8.5a.5.5 0 1 0 .998-.06l-.5-8.5a.5.5 0 1 0-.998.06Zm3 .5a.5.5 0 0 0-1 0v8.5a.5.5 0 0 0 1 0v-8.5Z"/>
                    </svg>
                </button>` : ''}
                ...
                <div class="flex flex-col h-full mt-2">
                    ...
                    <p class="font-bold text-lg text-slate-800 leading-tight mb-2 uppercase text-center">${escapeHTML(nomeSeguro).toUpperCase()}</p>

                    <div class="text-xs text-slate-600 mb-3 text-center">
                        <p>Assunto: <span class="font-bold uppercase text-slate-800">${escapeHTML(assuntoSeguro)}</span></p>
                    </div>
    ...
    createEmAtendimentoCard(item, currentPautaData, pautaId, userName, index) {
        try {
            ...
            const historicoTransferenciaHtml = item.historicoTransferencia
                ? `<div class="mt-2 bg-orange-50 border border-orange-200 text-orange-800 text-[10px] p-2 rounded-lg flex items-center justify-center gap-1 font-medium shadow-sm mb-2 text-center">
            ...
            if (item.selectedAction) {
                ...
                docStatusHtml = `
                    <div class="mt-3 mb-2 flex flex-col gap-1 items-center justify-center">
                        <span class="text-[10px] font-bold text-blue-800 bg-blue-50 px-2 py-0.5 rounded border border-blue-100 truncate flex items-center gap-1">
                            <span>📂</span>
                            <span>${escapeHTML(item.selectedAction)}</span>
                        </span>
            ...
            card.innerHTML = `
                ...
                <div class="text-center pt-2">
                    ${badgeAgendamentoHtml}
                    <p class="font-bold text-lg text-slate-800 leading-tight uppercase mb-2">${index + 1}. ${escapeHTML(item.name || '').toUpperCase()}</p>
                    <p class="text-xs text-slate-600 mb-1">Assunto: <strong class="uppercase text-slate-800">${escapeHTML(item.subject || 'Não informado')}</strong></p>
        ...
        try {
            ...
            card.innerHTML = `
                <div class="absolute top-3 right-3">
                    <button data-id="${item.id}" class="toggle-confirmed-atendido w-7 h-7 rounded-full border border-slate-200 flex items-center justify-center ${confirmButton} shadow-sm" ${canToggleConfirmed ? '' : 'disabled'}>
                ...
                <div class="text-center pt-2">
                    ${badgeAgendamentoHtml}
                    <p class="font-bold text-lg text-slate-800 leading-tight uppercase mb-2 px-6">${escapeHTML(item.name || '').toUpperCase()}</p>
                    <p class="text-xs text-slate-600 mb-2">Assunto: <strong class="uppercase text-slate-800">${escapeHTML(item.subject || 'Não informado')}</strong></p>
                    ...
                    <div class="flex items-center justify-center gap-2 mb-3 w-full">
                        <div class="flex flex-col items-center bg-slate-50 border border-slate-100 rounded-lg p-1.5 flex-1">
                            <span class="text-[9px] text-slate-400 uppercase font-bold tracking-wider mb-0.5">Agendado</span>
                            <span class="text-xs font-semibold text-slate-700">${item.scheduledTime || 'N/A'}</span>
                        </div>
                        <div class="flex flex-col items-center bg-slate-50 border border-slate-100 rounded-lg p-1.5 flex-1">
                        ...
                        <div class="flex flex-col items-center bg-emerald-50 border border-emerald-100 rounded-lg p-1.5 flex-1">
                ...
                <div class="flex flex-col items-center text-xs mb-4 pt-2 border-t border-slate-100">
                    <p class="text-slate-500 mb-2">Por: <b class="text-slate-800">${escapeHTML(atendenteNome)}</b></p>
                    <div class="flex flex-wrap justify-center gap-x-4 gap-y-2">
                        <button data-id="${item.id}" class="manage-demands-btn text-blue-600 font-bold hover:underline" ${canManageDemandsOrEditAttendant ? '' : 'disabled'}>Demandas</button>
                        <button data-id="${item.id}" class="edit-assisted-btn text-slate-500 font-bold hover:underline" ${canManageDemandsOrEditAttendant ? '' : 'disabled'}>Dados</button>
                        <button data-id="${item.id}" class="edit-attendant-btn text-emerald-600 font-bold hover:underline" ${canManageDemandsOrEditAttendant ? '' : 'disabled'}>Atendente</button>
    ...
    renderFaltososColumn(items) {
        ...
        items.forEach(item => {
            ...
            card.innerHTML = `
                ...
                <div class="text-center pt-2">
                    ${badgeAgendamentoHtml}
                    <p class="font-bold text-lg text-slate-800 leading-tight uppercase px-8">${escapeHTML(item.name || '').toUpperCase()}</p>
                    <span class="text-[9px] font-black text-red-700 bg-red-100 px-2 py-0.5 rounded-md mt-2 border border-red-200 inline-block uppercase tracking-wider">🚫 Faltoso</span>
                    
                    <p class="text-xs text-slate-700 mt-3 mb-2">Assunto: <b class="uppercase">${escapeHTML(item.subject || 'Não informado')}</b></p>
                </div>
                ...
                <div class="flex flex-col items-center text-[10px] md:text-xs mb-4 pt-2 border-t border-red-100">
                    <p class="text-slate-500 italic mb-2">Status Verde: <span class="${isConfirmed ? 'text-emerald-600' : 'text-amber-600'} font-bold">${isConfirmed ? 'Lançado no Verde' : 'Pendente de Lançamento'}</span></p>
                    <div class="flex justify-center gap-4">
                        <button data-id="${item.id}" class="edit-assisted-btn text-slate-500 font-bold hover:underline" ${canRevert ? '' : 'disabled'}>Editar Dados</button>
                        <button data-id="${item.id}" class="delete-btn text-red-500 font-bold hover:underline" ${canDelete ? '' : 'disabled'}>Deletar Faltoso</button>
                    </div>
                </div>
        ...
        Object.keys(groups).forEach(defensor => {
            ...
            groups[defensor].forEach((item, index) => {
                ...
                const deleteBtnHtml = canDelete ? `
                    <button data-id="${item.id}" class="delete-btn absolute top-2 left-8 text-slate-300 hover:text-red-500 transition-colors bg-white rounded-full">
                        <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" fill="currentColor" viewBox="0 0 16 16">
                            <path d="M11 1.5v1h3.5a.5.5 0 0 1 0 1h-.538l-.853 10.66A2 2 0 0 1 11.115 16h-6.23a2 2 0 0 1-1.994-1.84L2.038 3.5H1.5a.5.5 0 0 1 0-1H5v-1A1.5 1.5 0 0 1 6.5 0h3A1.5 1.5 0 0 1 11 1.5Zm-5 0v1h4v-1a.5.5 0 0 0-.5-.5h-3a.5.5 0 0 0-.5.5ZM4.5 5.029l.5 8.5a.5.5 0 1 0 .998-.06l-.5-8.5a.5.5 0 1 0-.998.06Zm3 0l.5 8.5a.5.5 0 1 0 .998-.06l-.5-8.5a.5.5 0 1 0-.998.06Zm3 .5a.5.5 0 0 0-1 0v8.5a.5.5 0 0 0 1 0v-8.5Z"/>
                        </svg>
                    </button>` : '';
                ...
                const actionControlsHtml = canManageDistribution
                    ? `<div class="mt-4 flex flex-col gap-2">
                            ...
                            <button data-id="${item.id}" class="return-to-aguardando-from-dist-btn w-full bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold py-2 rounded-lg text-xs shadow-sm transition active:scale-95 uppercase tracking-wide border border-slate-200">
                                Reverter para Fila
                            </button>
                       </div>`
                    : `<div class="mt-4">
