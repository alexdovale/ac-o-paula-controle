// js/ui.js - VERSÃO COMPLETA E ATUALIZADA (com todas as funções originais, melhorias e CORREÇÕES DO MODAL)
...
export const UIService = {
    ...
    // ============================================================
    // FUNÇÕES RESTAURADAS PARA O MODAL DE COLABORADORES
    // ============================================================
    preencherListaColaboradoresModal(app) {
        // Suporta ambos os IDs para garantir que encontre o container
    ...
    destacarSelecao(container, btnSelecionado) {
        ...
        container.querySelectorAll('button.collaborator-item').forEach(b => {
    ...
    preencherSelectColaboradores(app, selectId) {
        ...
        if (app.colaboradores && app.colaboradores.length > 0) {
            app.colaboradores.forEach(c => {
                ...
                opt.value = c.nome;
                opt.textContent = `${c.nome} (${c.cargo || 'Membro'})`;
    ...
    createAguardandoCard(item, currentPautaData, colaboradores, index) {
        try {
            ...
            const card = document.createElement('div');
            const priorityClass = PautaService?.getPriorityClass ? PautaService.getPriorityClass(item.priority) : '';
