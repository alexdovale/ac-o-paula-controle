...
// js/pdfService.js - VERSÃO DEFINITIVA (SIGEP + Preview Ata + Nova Planilha de Gastos Detalhada)
        ...
        try {
            ...
            categorias.forEach(cat => {
                ...
                if (valor && valor !== 'R$ 0,00' && String(valor).trim() !== '') {
                    ...
                    // Mostra linha em branco para preenchimento manual
                    body.push([
                        { content: cat.label, styles: { halign: 'left', valign: 'middle', cellPadding: 4 } },
            ...
            doc.autoTable({
                startY: 110,
                head: [[
                    { content: 'DESCRIÇÃO', styles: { halign: 'center', fontStyle: 'bold', fillColor: [200, 220, 200], textColor: [0,0,0], lineWidth: 1, lineColor: [0,0,0] } },
                    { content: 'VALOR MENSAL', styles: { halign: 'center', fontStyle: 'bold', fillColor: [200, 220, 200], textColor: [0,0,0], lineWidth: 1, lineColor: [0,0,0] } }
                ]],
                body: body,
                theme: 'grid',
    ...
    async generateChecklistPDF(assistedName, actionTitle, checklistData, documentosTextos) {
        try {
            ...
            if (checklistData.expenseData && checklistData.expenseData.checkExibirGastos) {
                ...
                // ⭐ NOVA PLANILHA DE GASTOS DETALHADA
