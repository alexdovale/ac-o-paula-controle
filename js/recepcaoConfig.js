...
// - Configurar preferências do painel público (modo e vídeo)
...
//     membros: ["uid1", "uid2"]        
//     verTudo: false                   
//     modoVisualizacao: "fila" // 'fila' | 'tv' | 'video' (NOVO)
//     videoUrl: "https://youtube..." // (NOVO)
//     ativo: true
//     criadoPor: "uid"
//     criadoEm: timestamp
// ──────────────────────────────────────────────────────────────────────────────
...
export const RecepcaoConfigService = {
    ...
    async criarRecepcao(db, dados, criadorUid) {
        try {
            ...
            const recepcao = {
                ...
                modoVisualizacao: dados.modoVisualizacao || 'fila', // Salva o modo
                videoUrl:         dados.videoUrl || '',             // Salva a URL do vídeo
                ativo:            true,
    ...
    _cardSelectorRecepcao(rec) {
        const corConfig = CORES[rec.cor] || CORES.slate;
        const tipoTag = rec.tipo === 'central' ? '<span class="text-[8px] font-black bg-amber-100 text-amber-700 px-2 py-0.5 rounded-full uppercase tracking-wider">Central</span>' : '';
        return `
            <button class="rc-selector-recepcao group text-left bg-white border-2 ${corConfig.border} hover:${corConfig.bg} rounded-2xl p-4 shadow-sm hover:shadow-lg transition-all duration-200"
                ...
                <div class="flex items-start gap-3">
                    ...
                    <div class="min-w-0 flex-1">
                        ...
                        ${rec.grupos && rec.grupos.length > 0 ? `<div class="flex flex-wrap gap-1 mt-2">${rec.grupos.map(g => this._tagGrupo(g, corConfig)).join('')}</div>` : ''}
                    </div>
                </div>
            </button>
        `;
        ...
        return `
            <div class="space-y-4">
                <div class="flex justify-between items-center">
                    ...
                    <button id="btn-nova-recepcao" class="bg-green-600 hover:bg-green-700 text-white font-bold px-4 py-2 rounded-lg text-sm transition shadow">+ Nova Recepção</button>
                </div>
                ${recepcoes.length === 0
                        ...
                        return `
                            <div class="bg-white border border-slate-200 rounded-xl p-4 flex items-center justify-between gap-4 shadow-sm">
                                ...
                                <div class="flex gap-2 shrink-0">
                                    <button class="btn-editar-recepcao bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold text-xs px-3 py-1.5 rounded-lg transition" data-recepcao-id="${rec.id}">✏️ Editar</button>
                                    <button class="btn-excluir-recepcao bg-red-50 hover:bg-red-100 text-red-600 font-bold text-xs px-3 py-1.5 rounded-lg transition" data-recepcao-id="${rec.id}" data-nome="${rec.nome}">🗑️</button>
                                </div>
                            </div>
                        `;
                    }).join('')
    ...
    renderFormRecepcao(recepcao = null, unidades = []) {
        ...
        const coresHtml = Object.entries(CORES).map(([key, val]) => `
            <button type="button" class="color-option w-8 h-8 rounded-full ${val.bg} ring-offset-2 transition-transform hover:scale-110 ${recepcao?.cor === key ? 'ring-2 ring-offset-2 scale-110 shadow-md' : ''}" data-cor="${key}"></button>
        `).join('');

        const iconesHtml = Object.entries(ICONES_SUGERIDOS).map(([key, icone]) => `
            <button type="button" class="icone-option text-2xl p-2 rounded-xl hover:bg-slate-100 transition ${recepcao?.icone === icone ? 'bg-slate-100 ring-2 ring-slate-300' : ''}" data-icone="${icone}">${icone}</button>
        `).join('');
            ...
            <div class="space-y-5">
                ...
                <div>
                    ...
                    <input type="text" id="form-rec-nome" value="${recepcao?.nome || ''}"
                        class="w-full p-3 border border-slate-300 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-green-500" placeholder="Ex: Família - 2º Andar">
                </div>
                ...
                <div class="grid grid-cols-2 gap-3">
                    <div>
                        ...
                        <input type="text" id="form-rec-unidade-nome" value="${recepcao?.unidadeNome || ''}"
                            class="w-full p-3 border border-slate-300 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-green-500" placeholder="Ex: DP Duque de Caxias">
                    ...
                    <div>
                        ...
                        <input type="text" id="form-rec-andar" value="${recepcao?.andar || ''}"
                            class="w-full p-3 border border-slate-300 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-green-500" placeholder="Ex: 2º Andar">
                    </div>
                </div>
                ...
                <div id="form-rec-grupos-wrap">
                    ...
                    <div class="flex gap-2 mb-2">
                        <input type="text" id="form-rec-grupo-input" class="flex-1 p-2.5 border border-slate-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-green-500" placeholder="Ex: familia, civel, criminal...">
                        <button type="button" id="btn-add-grupo" class="bg-slate-800 text-white font-bold px-4 py-2 rounded-lg text-sm hover:bg-slate-900 transition">+ Adicionar</button>
                    </div>
                    <div id="form-rec-grupos-lista" class="flex flex-wrap gap-2 min-h-[32px]">
                        ${(recepcao?.grupos || []).map(g => `
                            <span class="bg-slate-100 text-slate-700 border border-slate-200 text-xs font-bold px-3 py-1 rounded-full flex items-center gap-1">
                                ${g} <button type="button" class="btn-remove-grupo text-slate-400 hover:text-red-500 transition font-black" data-grupo="${g}">×</button>
                            </span>
                        `).join('')}
                    </div>
                </div>
                ...
                <div class="flex items-center gap-3 p-3 bg-amber-50 border border-amber-200 rounded-xl">
                    <input type="checkbox" id="form-rec-vertudo" ${recepcao?.verTudo ? 'checked' : ''} class="w-4 h-4 text-amber-600 rounded cursor-pointer">
                    <div>
                ...
                <!-- SEÇÃO NOVA: Configurações do Painel Público -->
                <div class="mt-4 pt-4 border-t border-slate-200 bg-slate-50 p-4 rounded-xl border">
                    <h4 class="text-xs font-black text-slate-700 uppercase tracking-widest mb-3">📺 Configurações do Painel Público</h4>
                    ...
                    <div class="mb-4">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-3 gap-2">
                            <label class="flex items-center gap-2 p-2.5 border border-slate-200 bg-white rounded-lg cursor-pointer hover:border-green-400 transition">
                                <input type="radio" name="form-rec-modo" value="fila" ${!recepcao || recepcao.modoVisualizacao === 'fila' || !recepcao.modoVisualizacao ? 'checked' : ''} class="text-green-600 w-4 h-4">
                                <span class="text-xs font-bold text-slate-700">📋 Fila (Lista)</span>
                            </label>
                            <label class="flex items-center gap-2 p-2.5 border border-slate-200 bg-white rounded-lg cursor-pointer hover:border-green-400 transition">
                                <input type="radio" name="form-rec-modo" value="tv" ${recepcao?.modoVisualizacao === 'tv' ? 'checked' : ''} class="text-green-600 w-4 h-4">
                                <span class="text-xs font-bold text-slate-700">📺 TV (Clássica)</span>
                            </label>
                            <label class="flex items-center gap-2 p-2.5 border border-slate-200 bg-white rounded-lg cursor-pointer hover:border-green-400 transition">
                                <input type="radio" name="form-rec-modo" value="video" ${recepcao?.modoVisualizacao === 'video' ? 'checked' : ''} class="text-green-600 w-4 h-4">
                                <span class="text-xs font-bold text-slate-700">🎬 TV + Vídeo</span>
                            </label>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <input type="text" id="form-rec-video" value="${recepcao?.videoUrl || ''}"
                            class="w-full p-2.5 border border-slate-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-green-500 bg-white"
                            placeholder="Ex: https://www.youtube.com/watch?v=...">
                        <p class="text-[10px] text-slate-500 mt-1 font-medium">Este vídeo ficará passando na tela quando o modo "TV + Vídeo" for escolhido.</p>
                    </div>
                </div>

                <div class="flex gap-3 pt-2">
                    <button type="button" id="btn-cancelar-form-rec" class="flex-1 bg-slate-200 hover:bg-slate-300 text-slate-700 font-bold py-3 rounded-xl transition text-sm">Cancelar</button>
                    <button type="button" id="btn-salvar-recepcao" class="flex-1 bg-green-600 hover:bg-green-700 text-white font-black py-3 rounded-xl transition text-sm shadow" data-recepcao-id="${recepcao?.id || ''}">
    ...
    initFormRecepcaoEventos(onSalvar, onCancelar) {
        ...
        document.getElementById('btn-salvar-recepcao')?.addEventListener('click', () => {
            ...
            const unidadeId = document.getElementById('form-rec-unidade-id')?.value || unidNome?.toLowerCase().replace(/\s+/g, '_').replace(/[^a-z0-9_]/g, '');
