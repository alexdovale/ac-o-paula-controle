...
// --- 1. CONSTANTES E BASE DE DADOS (VERSÃO INTEGRAL MANTIDA) ---
...
function renderReuForm() {
    const showWorkInfo = ACTIONS_WITH_WORK_INFO.includes(currentChecklistAction);
    const div = document.createElement('div');
    div.id = 'dynamic-reu-form';
    div.className = 'p-6 bg-blue-50 border-2 border-blue-200 rounded-2xl shadow-sm';
    div.innerHTML = `
        ...
        ${showWorkInfo ? `
            <div class="mt-4 pt-4 border-t border-blue-100">
                <label class="text-[9px] font-black text-blue-400 uppercase mb-1">Local de Trabalho / Renda</label>
                <input type="text" id="empresa-reu" placeholder="Empresa ou profissão" class="w-full p-2 border rounded-lg bg-white mb-2 shadow-sm">
    ...
    const cepInp = div.querySelector('#cep-reu');
    if (cepInp) {
        cepInp.onblur = async () => {
    ...
    const checkReuVisibility = () => {
        ...
        if (reuContainer) {
            ...
            if (needsReu) {
                ...
                reuContainer.appendChild(renderReuForm());
...
async function handleSave() {
    ...
    const checkedIds = Array.from(document.querySelectorAll('.doc-checkbox:checked')).map(cb => cb.id);
    const docTypes = {};
...
async function handleReset() {
    if (!confirm("Isso apagará todo o progresso. Mudar assunto?")) return;
