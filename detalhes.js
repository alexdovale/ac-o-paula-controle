...
// --- 1. CONSTANTES E BASE DE DADOS INTEGRAL ---
    ...
    container.innerHTML = `
        <div class="p-4 sm:p-6 bg-blue-50 border-2 border-blue-200 rounded-2xl shadow-sm mt-6">
            ...
            <div class="mt-4 grid grid-cols-2 sm:grid-cols-3 gap-2">
                <div class="col-span-1"><label class="text-[9px] font-black text-blue-500 uppercase font-bold">CEP</label><input type="text" id="cep-reu" maxlength="9" placeholder="00000-000" class="w-full p-2 border-2 border-blue-200 rounded-lg bg-white font-bold text-blue-700 shadow-sm"></div>
...
function renderChecklist(actionKey) {
            ...
            if (typeof docItem === 'object' && docItem.type === 'title') {
                ...
                li.innerHTML = `
                    <div class="flex flex-col border-b border-gray-50 pb-1">
                        ...
                        <div id="type-${id}" class="ml-10 mt-1 flex gap-6 ${isChecked ? '' : 'hidden'}">
    ...
    const checkReuVisibility = () => {
        ...
        const addressContainer = getEl('address-editor-container');
        if (addressContainer) {
            addressContainer.classList.toggle('hidden', !needsReu);
            if (needsReu) {
...
function renderExpenseTable() {
    ...
    EXPENSE_CATEGORIES.forEach(c => {
        // VISUALIZAÇÃO COM DESCRIÇÃO VISÍVEL ABAIXO DO TÍTULO
        rows += `
            <tr class="border-b border-green-100 last:border-0">
                <td class="py-3">
                    ...
                    <p class="text-[9px] text-green-600 font-normal mt-1 leading-tight italic opacity-75">${c.desc}</p>
                </td>
                <td class="py-3 pl-2">
                    <input type="text" id="expense-${c.id}" class="expense-input w-full p-2 bg-white border border-green-200 rounded-lg text-right text-xs shadow-sm focus:ring-2 focus:ring-green-400 outline-none" placeholder="R$ 0,00">
                </td>
            </tr>`;
    });
...
async function handleSave() {
    ...
    const payload = {
        documentChecklist: {
            ...
            checkedIds: Array.from(container.querySelectorAll('.doc-checkbox:checked')).map(cb => cb.id),
            docTypes: (() => {
                const types = {};
                container.querySelectorAll('.doc-checkbox:checked').forEach(cb => {
                    types[cb.id] = document.querySelector(`input[name="type-${cb.id}"]:checked`)?.value || 'Físico';
                });
                return types;
            })(),
...
async function handlePdf() {
    try {
        ...
        docPDF.text(`Relatório SIGAP - ${assistedNameEl.textContent}`, 15, 20);
        docPDF.save(`Checklist_SIGAP.pdf`);
    } catch (err) { console.error(err); }
}

async function handleReset() {
    if (!confirm("Isso apagará o checklist e os dados do réu. Mudar assunto?")) return;
    try {
